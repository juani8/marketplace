...
import ProductTable from './ProductTable'; // Asegurate de importar esto
...
export default function PromotionForm({
  ...
  const validate = () => {
    ...
    if (formData.tipo_promocion === 'monto') {
      const productosExcedidos = products.filter(
        (p) =>
          formData.productos_incluidos.includes(p.producto_id) &&
    ...
    if (formData.productos_incluidos.length === 0) {
  ...
  const handleChangeWrapper = (e) => {
    ...
    if (type === 'number') {
      ...
      // Restricción: Si es porcentaje, no dejar más de 100
      if (
  ...
  const toggleProduct = (id) => {
    const nuevos = formData.productos_incluidos.includes(id)
      ? formData.productos_incluidos.filter((p) => p !== id)
      : [...formData.productos_incluidos, id];
    handleChange({ target: { name: 'productos_incluidos', value: nuevos } });
  };
  ...
  const productosSinPromocion = products.filter(
    (p) => !p.promociones || p.promociones.length === 0
  );
  ...
  const calcularPrecioConDescuento = (precioOriginal) => {
    ...
    if (formData.tipo_promocion === 'porcentaje') {
    ...
    >
      ...
      <Input
        ...
        max={
            ...
            : Math.min(
                ...products
                    .filter((p) => formData.productos_incluidos.includes(p.producto_id))
                    .map((p) => p.precio),
                Infinity
                )
        }
        ...
        {isFetchingProducts ? (
            ...
            {productosSinPromocion.map((p) => (
                ...
                <input
                    type="checkbox"
                    checked={formData.productos_incluidos.includes(p.producto_id)}
